...
exports.LUA_API = {
  ...
  async subscribeOne(L, T) {
    ...
    // const sub = await entry.subscribe(0, 'TODO');
    const subDevice = new pollable.PollableSubscribeOne(entry/*, sub*/);
  ...
  async poll(L, T) {
    ...
    // read arguments
    const millis = Math.floor(lauxlib.luaL_checknumber(L, 2));
    const pollables = this.readLuaEntry(T, 1);
    lua.lua_settop(L, 0);

    // TODO
    console.log('lua be tryin to poll', pollables, 'for', millis, 'seconds');
    throw new Error(`TODO: ctx.poll()`);
  },
